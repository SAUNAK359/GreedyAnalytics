...
from analytics_llm.backend.core.security import SecurityHeaders
from analytics_llm.backend.core.rate_limit import RateLimiter
from analytics_llm.backend.auth.auth_service import AuthService
from analytics_llm.backend.auth.tokens import decode_token, JWTError, create_access_token, create_refresh_token
from analytics_llm.backend.auth.rbac import authorize
from analytics_llm.backend.auth.models import User
from analytics_llm.backend.storage.postgres import get_db, Base, engine
from analytics_llm.backend.tenants.tenant_models import Tenant
from analytics_llm.backend.llm.router import LLMRouter
from analytics_llm.backend.dashboard.mcp_engine import MCPEngine
from analytics_llm.backend.observability.telemetry import init_telemetry
...
from analytics_llm.backend.ingestion.datasource import DataSourceManager
from analytics_llm.backend.agents.orchestrator import AgentOrchestrator
...
async def analytics_query(
    ...
    try:
        ...
        if result.get("error") == "TOKEN_BUDGET_EXCEEDED":
            raise HTTPException(
                status_code=status.HTTP_429_TOO_MANY_REQUESTS,
                detail="Token budget exceeded"
            )

        if result.get("error") == "COST_BUDGET_EXCEEDED":
            raise HTTPException(
                status_code=status.HTTP_429_TOO_MANY_REQUESTS,
